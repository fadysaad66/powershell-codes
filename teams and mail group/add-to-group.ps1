    ...
    foreach ($entry in $emails) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($userEmail)) {
            ...
            try {
                ...
                Write-Host "Successfully added $userEmail to $groupId." -ForegroundColor Green
            } catch {
...
AddEmailsToGroup -groupId $groupId -excelFilePath $excelFilePath
